...
CHECKER_NAME = noVirtualInheritTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h noVirtualInheritTest
...
noVirtualInheritTest: noVirtualInherit.C noVirtualInheritMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ noVirtualInheritMain.C noVirtualInherit.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: noVirtualInheritTest noVirtualInheritTest1.C
	./noVirtualInheritTest noVirtualInheritTest1.C

clean:
	rm -f libcompass.so "noVirtualInheritTest"
