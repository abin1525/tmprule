...
CHECKER_NAME = charVarWithCPrefixTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h charVarWithCPrefixTest
...
charVarWithCPrefixTest: charVarWithCPrefix.C charVarWithCPrefixMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ charVarWithCPrefixMain.C charVarWithCPrefix.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: charVarWithCPrefixTest charVarWithCPrefixTest1.C
	./charVarWithCPrefixTest charVarWithCPrefixTest1.C

clean:
	rm -f libcompass.so "charVarWithCPrefixTest"
