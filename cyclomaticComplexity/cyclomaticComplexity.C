...
std::string shortDescription = "This function is too complex : CC = ";
...
CompassAnalyses::CyclomaticComplexity::CheckerOutput::CheckerOutput(
		SgNode* node, const std::string & reason) :
		OutputViolationBase(node, checkerName,
				reason + shortDescription + Traversal::getComplexity() + " > "
						+ Traversal::getCC()) {
}
...
CompassAnalyses::CyclomaticComplexity::Traversal::Traversal(
	...
	cc = Compass::parseInteger(
			inputParameters["CyclomaticComplexity.maxComplexity"]);
...
void CompassAnalyses::CyclomaticComplexity::Traversal::checkNode(SgNode* node) {
	if (isSgIfStmt(node) || isSgCaseOptionStmt(node) || isSgForStatement(node)
			|| isSgDoWhileStmt(node) || isSgWhileStmt(node)) {
		complexity++;
	}
}

int CompassAnalyses::CyclomaticComplexity::Traversal::checkDepth(
...
void CompassAnalyses::CyclomaticComplexity::Traversal::visit(SgNode* node) {
	if (isSgFunctionDeclaration(node)) {
		...
		std::vector<SgNode*> children =
				funcDecl->get_traversalSuccessorContainer();
		checkDepth(children);
		if (complexity > cc)
			output->addOutput(
					new CheckerOutput(funcDecl,
							"\t" + funcDecl->get_name() + "\t"));
	}
} //End of the visit function.

static void run(Compass::Parameters params, Compass::OutputObject* output) {
	CompassAnalyses::CyclomaticComplexity::Traversal(params, output).run(
