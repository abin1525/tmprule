...
CHECKER_NAME = binaryBufferOverflowTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h binaryBufferOverflowTest
...
binaryBufferOverflowTest: binaryBufferOverflow.C binaryBufferOverflowMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ binaryBufferOverflowMain.C binaryBufferOverflow.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: binaryBufferOverflowTest binaryBufferOverflowTest1.C
	./binaryBufferOverflowTest binaryBufferOverflowTest1.C

clean:
	rm -f libcompass.so "binaryBufferOverflowTest"
