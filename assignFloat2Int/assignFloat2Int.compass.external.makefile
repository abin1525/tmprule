...
CHECKER_NAME = assignFloat2IntTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h assignFloat2IntTest
...
assignFloat2IntTest: assignFloat2Int.C assignFloat2IntMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ assignFloat2IntMain.C assignFloat2Int.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: assignFloat2IntTest assignFloat2IntTest1.C
	./assignFloat2IntTest assignFloat2IntTest1.C

clean:
	rm -f libcompass.so "assignFloat2IntTest"
