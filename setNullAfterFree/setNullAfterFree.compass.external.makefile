...
CHECKER_NAME = setNullAfterFreeTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h setNullAfterFreeTest
...
setNullAfterFreeTest: setNullAfterFree.C setNullAfterFreeMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ setNullAfterFreeMain.C setNullAfterFree.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: setNullAfterFreeTest setNullAfterFreeTest1.C
	./setNullAfterFreeTest setNullAfterFreeTest1.C

clean:
	rm -f libcompass.so "setNullAfterFreeTest"
