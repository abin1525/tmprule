...
CHECKER_NAME = readBufferOverflowTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h readBufferOverflowTest
...
readBufferOverflowTest: readBufferOverflow.C readBufferOverflowMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ readBufferOverflowMain.C readBufferOverflow.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: readBufferOverflowTest readBufferOverflowTest1.C
	./readBufferOverflowTest readBufferOverflowTest1.C

clean:
	rm -f libcompass.so "readBufferOverflowTest"
