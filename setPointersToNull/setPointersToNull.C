...
const std::string shortDescription =
		"Set all dynamically allocated pointers to NULL after calling free().";
...
CompassAnalyses::SetPointersToNull::CheckerOutput::CheckerOutput(SgNode* node,
...
void CompassAnalyses::SetPointersToNull::Traversal::visit(SgNode* node) {
		...
		if (func_name_str.compare("free") == 0) {
			...
			case V_SgArrowExp: {
				...
				if (isSgVarRefExp(ex))
					...
					cerr
							<< "1: setPointersToNULL : isSgVarRefExp not found in isSgArrowExp "
							<< endl;
				break;
			}
			case V_SgDotExp: {
				...
				if (isSgVarRefExp(ex))
					...
					cerr
							<< "1: setPointersToNULL : isSgVarRefExp not found in isSgDotExp "
							<< endl;
				break;
			}
			case V_SgPointerDerefExp: {
				...
				if (isSgVarRefExp(ex))
					...
					cerr
							<< "1: setPointersToNULL : isSgVarRefExp not found in isSgPointerDerefExp "
							<< endl;
				break;
			}
			default: {
				cerr << "1: Left hand side of (assign) free is unknown: "
						<< argument->class_name() << endl;
			}
			} // switch
			...
			if (var)
				...
				cerr
						<< " 1: No var -- Left hand side of (assign) free is unknown: "
						<< argument->class_name() << endl;
			...
			if (value == 0) {
				...
				case V_SgArrowExp: {
					...
					if (isSgVarRefExp(ex))
						...
						cerr
								<< "setPointersToNULL : isSgVarRefExp not found in isSgArrowExp "
								<< endl;
					break;
				}
				case V_SgDotExp: {
					...
					if (isSgVarRefExp(ex))
						...
						cerr
								<< "setPointersToNULL : isSgVarRefExp not found in isSgDotExp "
								<< endl;
					break;
				}
				case V_SgPointerDerefExp: {
					...
					if (isSgVarRefExp(ex))
						...
						cerr
								<< "setPointersToNULL : isSgVarRefExp not found in isSgPointerDerefExp "
								<< endl;
					break;
				}
				default: {
					cerr << "Left hand side of (assign) free is unknown: "
							<< assign->get_lhs_operand()->class_name() << endl;
				}
				} // switch
				if (var)
					...
					cerr
							<< " No var -- Left hand side of (assign) free is unknown: "
							<< assign->get_lhs_operand()->class_name() << endl;
...
static void run(Compass::Parameters params, Compass::OutputObject* output) {
	CompassAnalyses::SetPointersToNull::Traversal(params, output).run(
