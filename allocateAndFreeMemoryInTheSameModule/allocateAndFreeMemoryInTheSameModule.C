...
{ 
  ...
  if (aop) {
	...
	case V_SgVarRefExp: {
		var = isSgVarRefExp(aop->get_lhs_operand())->get_symbol();
		break;
	}
	case V_SgArrowExp: {
		SgExpression* ex =
				isSgArrowExp(aop->get_lhs_operand())->get_lhs_operand();
		if (isSgVarRefExp(ex))
			var = isSgVarRefExp(ex)->get_symbol();
		else
			cerr
					<< "allocateAndFreeMemory : isSgVarRefExp not found in isSgArrowExp "
					<< endl;
		break;
	}
	case V_SgDotExp: {
		SgExpression* ex =
				isSgDotExp(aop->get_lhs_operand())->get_lhs_operand();
		if (isSgVarRefExp(ex))
			var = isSgVarRefExp(ex)->get_symbol();
		else
			cerr
					<< "allocateAndFreeMemory : isSgVarRefExp not found in isSgDotExp "
					<< endl;
		break;
	}
	case V_SgPointerDerefExp: {
		SgExpression* ex =
				isSgPointerDerefExp(aop->get_lhs_operand())->get_operand();
		if (isSgVarRefExp(ex))
			var = isSgVarRefExp(ex)->get_symbol();
		else
			cerr
					<< "allocateAndFreeMemory : isSgVarRefExp not found in isSgPointerDerefExp "
					<< endl;
		break;
	}
	default: {
		cerr << "Left hand side of malloc is unknown: "
				<< aop->get_lhs_operand()->class_name() << endl;
	}
	}
      if (var)
