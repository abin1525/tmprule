...
class Traversal: public AstTopDownBottomUpProcessing<void*, void*> {
private:
	...
	struct Var {
		...
		bool is_const;
		Var(bool is_const) :
				is_used_in_scope(false), is_used_close(false), is_far(false), was_used(
						false), is_const(is_const) {
		}
	};
...
CompassAnalyses::LocalizedVariables::CheckerOutput::CheckerOutput(SgNode* node,
		std::string msg) :
...
void*
CompassAnalyses::LocalizedVariables::Traversal::evaluateInheritedAttribute(
		SgNode* n, void*) {
	...
	if (decl != NULL) {
		for (SgInitializedNamePtrList::iterator i =
				decl->get_variables().begin(); i != decl->get_variables().end();
				++i) {
			//Is it a constant?
			SgModifierType* mod = isSgModifierType((*i)->get_type());
			bool cons = false;
			if (mod != NULL)
				if (mod->get_typeModifier().get_constVolatileModifier().isConst())
					cons = true;
...
void*
CompassAnalyses::LocalizedVariables::Traversal::evaluateSynthesizedAttribute(
		SgNode* n, void*, SynthesizedAttributesList) {
	//Quitting a scope, we get the report of usage of each variable
	//declared in the scope we are quitting and then clean the
	//environment.
	if (isSgScopeStatement(n) != NULL) {
		map* top = stack.top();
		for (map::iterator i = top->begin(); i != top->end(); ++i) {
			if ((*i).second != NULL) {
				if (!((*i).second->was_used)) {
					//We have never used the variable.
					output->addOutput(new OutputNever((*i).first));
				} else {
					if (!((*i).second->is_const)) {
						...
						if (!((*i).second->is_used_in_scope)) {
							output->addOutput(new OutputWide((*i).first));
						} else if (!((*i).second->is_used_close)) {
							output->addOutput(new OutputFar((*i).first));
						}
					}
				}
				delete (*i).second;
	...
	if (isSgStatement(n) != NULL) {
		...
		map* scope = stack.top();
		if (scope != NULL) {
			for (map::iterator j = scope->begin(); j != scope->end(); ++j) {
				Var* v = (*j).second;
				if (v != NULL) {
...
static void run(Compass::Parameters params, Compass::OutputObject* output) {
	CompassAnalyses::LocalizedVariables::Traversal(params, output).run(
			Compass::projectPrerequisite.getProject());
}

extern const Compass::Checker* const localizedVariablesChecker =
		new Compass::Checker(CompassAnalyses::LocalizedVariables::checkerName,
