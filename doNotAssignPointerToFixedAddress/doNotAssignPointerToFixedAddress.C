...
CompassAnalyses::DoNotAssignPointerToFixedAddress::CheckerOutput::CheckerOutput(
		SgNode* node, std::string message) :
		OutputViolationBase(node,
				::doNotAssignPointerToFixedAddressChecker->checkerName, message) {
}

CompassAnalyses::DoNotAssignPointerToFixedAddress::Traversal::Traversal(
		Compass::Parameters inputParameters, Compass::OutputObject* output) :
		output(output) {
}

bool CompassAnalyses::DoNotAssignPointerToFixedAddress::is_pointer(
		SgType* node) {
	...
	return node
			&& (isSgPointerType(node)
					|| (typedef_node
							&& is_pointer(typedef_node->get_base_type())));
}

//----------------------------------------------------------------------------
// visit
void CompassAnalyses::DoNotAssignPointerToFixedAddress::Traversal::visit(
		SgNode* node) {
...
void CompassAnalyses::DoNotAssignPointerToFixedAddress::Traversal::visit(
		SgInitializedName* node) {
	if (is_pointer(node->get_type())
			&& is_illegal_constant(node->get_initializer())) {
		output->addOutput(
				new CheckerOutput(node,
						std::string(
								"Pointer " + node->get_name()
										+ " is initialized to a fixed address.")));
		return;
	}
	...
	if (arr
			&& is_pointer(
					arr->get_base_type()) && HAS_ILLEGAL_CONSTANT( isSgAggregateInitializer( node->get_initializer()))
					) {
		output->addOutput(
				new CheckerOutput(node,
						std::string(
								"Array of pointers " + node->get_name()
...
bool CompassAnalyses::DoNotAssignPointerToFixedAddress::is_zero_value(
		SgValueExp* value_node) {
	return (
...
bool CompassAnalyses::DoNotAssignPointerToFixedAddress::is_illegal_constant(
		SgInitializer* node) {
...
bool CompassAnalyses::DoNotAssignPointerToFixedAddress::is_constant(
		SgExpression* node) {
...
bool CompassAnalyses::DoNotAssignPointerToFixedAddress::has_illegal_constant(
...
static void run(Compass::Parameters params, Compass::OutputObject* output) {
	CompassAnalyses::DoNotAssignPointerToFixedAddress::Traversal(params, output).run(
...
static Compass::AstSimpleProcessingWithRunFunction* createTraversal(
	...
	return new CompassAnalyses::DoNotAssignPointerToFixedAddress::Traversal(
			params, output);
}
