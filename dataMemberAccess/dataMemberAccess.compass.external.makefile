...
CHECKER_NAME = dataMemberAccessTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h dataMemberAccessTest
...
dataMemberAccessTest: dataMemberAccess.C dataMemberAccessMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ dataMemberAccessMain.C dataMemberAccess.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: dataMemberAccessTest dataMemberAccessTest1.C
	./dataMemberAccessTest dataMemberAccessTest1.C

clean:
	rm -f libcompass.so "dataMemberAccessTest"
