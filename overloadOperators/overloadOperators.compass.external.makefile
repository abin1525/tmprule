...
CHECKER_NAME = overloadOperatorsTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h overloadOperatorsTest
...
overloadOperatorsTest: overloadOperators.C overloadOperatorsMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ overloadOperatorsMain.C overloadOperators.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: overloadOperatorsTest overloadOperatorsTest1.C
	./overloadOperatorsTest overloadOperatorsTest1.C

clean:
	rm -f libcompass.so "overloadOperatorsTest"
