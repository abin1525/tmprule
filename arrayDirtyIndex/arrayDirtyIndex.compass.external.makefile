...
CHECKER_NAME = arrayDirtyIndexTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h arrayDirtyIndexTest
...
arrayDirtyIndexTest: arrayDirtyIndex.C arrayDirtyIndexMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ arrayDirtyIndexMain.C arrayDirtyIndex.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: arrayDirtyIndexTest arrayDirtyIndexTest1.C
	./arrayDirtyIndexTest arrayDirtyIndexTest1.C

clean:
	rm -f libcompass.so "arrayDirtyIndexTest"
