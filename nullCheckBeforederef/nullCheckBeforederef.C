...
namespace NullCheckBeforederef {
...
class Traversal: public Compass::AstSimpleProcessingWithRunFunction {
	...
	void checkNullDeref(std::string analysisname, SgExpression* theExp,
			std::string name);
...
public:
	...
	void run(SgNode* n) {
		...
		counter = 0;
		debug = false;
		std::vector<SgNode*> exprList = NodeQuery::querySubTree(n,
...
bool CompassAnalyses::NullCheckBeforederef::Traversal::debug;

namespace CompassAnalyses {
namespace NullCheckBeforederef {
const std::string checkerName = "NullCheckBeforederef";
const std::string shortDescription = "Looks for Null Pointer Dereferences";
const std::string longDescription =
		"This analysis looks for common possible null pointer dereferences. It is based on BOOST.";
} //End of namespace NullCheckBeforederef.
} //End of namespace CompassAnalyses.

CompassAnalyses::NullCheckBeforederef::CheckerOutput::CheckerOutput(
		string problem, SgNode* node) :
		OutputViolationBase(node, checkerName, problem) {
}

CompassAnalyses::NullCheckBeforederef::Traversal::Traversal(
...
bool CompassAnalyses::NullCheckBeforederef::Traversal::isLegitimateNullPointerCheck(SgExpression* expr, SgInitializedName* pointerVar, bool invertCheck) const {
	if (debug) {
 ...
 ****************************************************************/
CompassAnalyses::NullCheckBeforederef::Traversal::BoolWithTrace CompassAnalyses::NullCheckBeforederef::Traversal::expressionIsNull(
		SgExpression* expr) {
	...
	CompassAnalyses::NullCheckBeforederef::Traversal::BoolWithTrace result(
			false, vector<SgExpression*>());
	switch (expr->variantT()) {
	case V_SgVarRefExp: {
	...
	default: {
		cout << "Compass::Nullderef::Unhandled expression kind "
 ...
 *********************************************************/
void CompassAnalyses::NullCheckBeforederef::Traversal::checkNullDeref(
		string analysisname, SgExpression* expr, string name) {
		...
		if (tr.first) {
			if (debug)
				cout << "Expression " << expr->unparseToString() << " at line "
						<< expr->get_startOfConstruct()->get_line()
						<< "  in file : "
						<< expr->get_file_info()->get_filename() << " ";
 ...
 *  to this analysis.
 *********************************************************/
void CompassAnalyses::NullCheckBeforederef::Traversal::visit(SgNode* sgNode) {
	...
	if (isSgArrowExp(sgNode)) {
		...
		const SgExpressionPtrList& args =
				delExpr->get_args()->get_expressions();
		if (delExpr != NULL) {
			if (args.size() >= 1) {
				SgFunctionRefExp* fr = isSgFunctionRefExp(
						delExpr->get_function());
				if (fr) {
					string name =
							fr->get_symbol()->get_declaration()->get_name().getString();
					if (name == "free") {
						if (debug)
							cerr << "\n >>>  Found SgFunctionCallExpr : "
									<< delExpr->unparseToString() << endl;
...
static void run(Compass::Parameters params, Compass::OutputObject* output) {
	CompassAnalyses::NullCheckBeforederef::Traversal(params, output).run(
...
static Compass::AstSimpleProcessingWithRunFunction* createTraversal(
	...
	return new CompassAnalyses::NullCheckBeforederef::Traversal(params, output);
}
...
extern const Compass::Checker* const nullCheckBeforederefChecker =
		new Compass::CheckerUsingAstSimpleProcessing(
				CompassAnalyses::NullCheckBeforederef::checkerName,
				CompassAnalyses::NullCheckBeforederef::shortDescription,
				CompassAnalyses::NullCheckBeforederef::longDescription,
