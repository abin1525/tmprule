...
CHECKER_NAME = nullCheckBeforederefTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h nullCheckBeforederefTest
...
nullCheckBeforederefTest: nullCheckBeforederef.C nullCheckBeforederefMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ nullCheckBeforederefMain.C nullCheckBeforederef.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: nullCheckBeforederefTest nullCheckBeforederefTest1.C
	./nullCheckBeforederefTest nullCheckBeforederefTest1.C

clean:
	rm -f libcompass.so "nullCheckBeforederefTest"
