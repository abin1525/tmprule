...
CHECKER_NAME = binaryInterruptAnalysisTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h binaryInterruptAnalysisTest
...
binaryInterruptAnalysisTest: binaryInterruptAnalysis.C binaryInterruptAnalysisMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ binaryInterruptAnalysisMain.C binaryInterruptAnalysis.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: binaryInterruptAnalysisTest binaryInterruptAnalysisTest1.C
	./binaryInterruptAnalysisTest binaryInterruptAnalysisTest1.C

clean:
	rm -f libcompass.so "binaryInterruptAnalysisTest"
