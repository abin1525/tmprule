...
CHECKER_NAME = placeConstantOnTheLhsTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h placeConstantOnTheLhsTest
...
placeConstantOnTheLhsTest: placeConstantOnTheLhs.C placeConstantOnTheLhsMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ placeConstantOnTheLhsMain.C placeConstantOnTheLhs.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: placeConstantOnTheLhsTest placeConstantOnTheLhsTest1.C
	./placeConstantOnTheLhsTest placeConstantOnTheLhsTest1.C

clean:
	rm -f libcompass.so "placeConstantOnTheLhsTest"
