...
CHECKER_NAME = constStringLiteralsTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h constStringLiteralsTest
...
constStringLiteralsTest: constStringLiterals.C constStringLiteralsMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ constStringLiteralsMain.C constStringLiterals.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: constStringLiteralsTest constStringLiteralsTest1.C
	./constStringLiteralsTest constStringLiteralsTest1.C

clean:
	rm -f libcompass.so "constStringLiteralsTest"
