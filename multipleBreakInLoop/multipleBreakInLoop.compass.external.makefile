...
CHECKER_NAME = multipleBreakInLoopTest

LINKER_FLAGS = -L$(ROSE_INSTALL)/lib -Wl,-rpath $(ROSE_INSTALL)/lib -lrose $(RT_LIBS) -L`pwd` -Wl,-rpath `pwd` -lcompass

all: prerequisites.h instantiate_prerequisites.h multipleBreakInLoopTest
...
multipleBreakInLoopTest: multipleBreakInLoop.C multipleBreakInLoopMain.C libcompass.so $(COMPASS_PROJECT)/src/compassSupport/compassTestMain.C
	g++ -fPIC -Wall -o $@ multipleBreakInLoopMain.C multipleBreakInLoop.C -I$(COMPASS_PROJECT)/src/compassSupport -I$(ROSE_INSTALL)/include -I$(COMPASS_PROJECT)/extensions/prerequisites -I. $(LINKER_FLAGS)

test: multipleBreakInLoopTest multipleBreakInLoopTest1.C
	./multipleBreakInLoopTest multipleBreakInLoopTest1.C

clean:
	rm -f libcompass.so "multipleBreakInLoopTest"
